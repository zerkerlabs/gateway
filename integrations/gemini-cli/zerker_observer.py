#!/usr/bin/env python3
"""Privacy-safe Gemini CLI hook adapter for Zerker Gateway."""
...
_STATE_ROOT = Path(tempfile.gettempdir()) / f"zerker-gemini-observer-{os.getuid()}"
...
def _event_id(session_ref: str, suffix: str) -> str:
    return "gemini-cli:" + _digest(f"{session_ref}:{suffix}")
...
def _resolve_agent_id() -> str:
    ...
    for agent in payload.get("agents", []):
        ...
        if metadata.get("zerker_discovery_key") == "gemini-cli":
            return str(agent["id"])
    raise RuntimeError("Gemini CLI is not enrolled in Zerker")
...
def _start_path(session_id: str, tool_name: str) -> Path:
    return _STATE_ROOT / _digest(session_id) / _digest(tool_name)


def _record_tool_start(session_id: str, tool_name: str) -> None:
    path = _start_path(session_id, tool_name)
    path.parent.mkdir(mode=0o700, parents=True, exist_ok=True)
    path.write_text(str(time.time_ns()), encoding="ascii")
    path.chmod(0o600)


def _tool_duration_ms(session_id: str, tool_name: str) -> int:
    path = _start_path(session_id, tool_name)
    try:
...
def handle_hook(payload: dict[str, Any]) -> None:
    ...
    timestamp = payload.get("timestamp") if isinstance(payload.get("timestamp"), str) else str(uuid.uuid4())
    ...
    if event_name == "BeforeTool":
        _record_tool_start(session_id, tool_name)
        return
    ...
    _spawn_emit({
        "session_ref": session_ref,
        "event_id": _event_id(session_ref, f"tool:{tool_name}:{timestamp}"),
        "type": "tool.completed",
        "tool_name": tool_name,
        "outcome": "failed" if failed else "succeeded",
        "duration_ms": _tool_duration_ms(session_id, tool_name),
    })
...
def main() -> int:
    try:
        ...
        # Measurement is fail-open. Gemini CLI must never wait on telemetry.
        pass
    print("{}")
    return 0
