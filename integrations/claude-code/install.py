#!/usr/bin/env python3
"""Install or remove the Zerker observer in Claude Code user hooks."""
...
def update_settings(settings: dict[str, Any], *, uninstall: bool = False) -> dict[str, Any]:
    ...
    for event in _EVENTS:
        ...
        if not isinstance(groups, list):
            raise ValueError(f"Claude Code hook {event!r} must be an array")
...
def main() -> int:
    ...
    print("Zerker Claude Code observer removed." if args.uninstall else "Zerker Claude Code observer installed.")
    return 0
