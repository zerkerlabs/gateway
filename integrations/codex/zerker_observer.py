#!/usr/bin/env python3
"""Privacy-safe Codex hook adapter for Zerker Gateway.

Codex sends hook payloads on stdin. This adapter extracts only lifecycle,
...
_STATE_ROOT = Path(tempfile.gettempdir()) / f"zerker-codex-observer-{os.getuid()}"
...
def _event_id(session_ref: str, suffix: str) -> str:
    return "codex:" + _digest(f"{session_ref}:{suffix}")
...
def _resolve_agent_id() -> str:
    ...
    for agent in payload.get("agents", []):
        ...
        if metadata.get("zerker_discovery_key") == "codex":
            return str(agent["id"])
    raise RuntimeError("Codex is not enrolled in Zerker")
